...
var _ = Describe("upUniformCanonicalIds", func() {
	...
	BeforeEach(func() {
		...
		// fork: app password rows must follow their user's id rewrite
		seed(`INSERT INTO user_app_password VALUES (?, ?)`, legacyOld, randOld)
	...
	It("canonicalizes every id family and keeps references consistent", func() {
		...
		Expect(get(`SELECT id FROM user_app_password`)).To(Equal(legacyNew))
		Expect(get(`SELECT user_id FROM user_app_password`)).To(Equal(randNew))
